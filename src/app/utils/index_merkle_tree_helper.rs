...
pub fn update_nullifier_tree<F: BigPrimeField>(
    ...
    let mut hasher = Poseidon::<F, 3, 2>::new(8, 57);
    let new_tree=MerkleTree::new(&mut hasher, hash_nullifier_pre_images(nullifier_tree_preimages.clone())).unwrap();
    ...
    let is_new_leaf_largest=if new_leaf.next_val==F::ZERO{F::from(1u64)}else{F::from(0u64)};
